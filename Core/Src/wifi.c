...
extern DMA_HandleTypeDef hdma_usart3_rx; // CHANGE THIS
...
void USER_UART_Handler(void) {
	if(__HAL_UART_GET_FLAG(wifiCom,UART_FLAG_IDLE) ==SET) { // Trigger idle interrupt
        ...
        temp = hdma_usart3_rx.Instance->CNDTR;   // CHANGE THIS, obtain untransferred data in DMA
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
	...
	if(huart->Instance == USART3) // CHANGE THIS
	{
		if(addr) {
